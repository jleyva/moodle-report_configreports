...
class template_form extends moodleform {
 function definition() {
        ...
        $optionsenabled = array(0=>get_string('disabled','report_configreports'),1=>get_string('enabled','report_configreports'));

        $mform->addElement('select','enabled',get_string('template','report_configreports'),$optionsenabled);
        $mform->setDefault('enabled',0);
    
        $mform->addElement('htmleditor', 'header', get_string('header', 'report_configreports'));
        $mform->disabledif ('header', 'enabled', 'eq', 0);
        $mform->addHelpButton('header','template_marks', 'report_configreports');
        ...
        $mform->addElement('static','statictext',get_string('availablemarks','report_configreports'),$availablemarksrec);
        $mform->addElement('htmleditor', 'record', get_string('templaterecord', 'report_configreports'));
        $mform->disabledif ('record', 'enabled', 'eq', 0);

        $mform->addElement('htmleditor', 'footer', get_string('footer', 'report_configreports'));
        $mform->disabledif ('footer', 'enabled', 'eq', 0);
        $mform->addHelpButton('footer','template_marks', 'report_configreports');
