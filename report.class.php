 ...
 */
 ...
 class report_base {
    ...
    function check_permissions($userid, $context) {
        ...
        if (has_capability('report/configreports:manageownreports', $context, $userid) && $this->config->ownerid == $userid)
            return true;
    
        if (has_capability('report/configreports:managereports', $context, $userid))
            return true;
        ...
        if (empty($permissions['elements'])) {
            return has_capability('report/configreports:viewreports', $context);
        }
        else{
            $finalresult = false;
            ...
            foreach ($permissions['elements'] as $p) {        
                ...
                require_once($CFG->dirroot.'/report/configreports/components/permissions/'.$p['pluginname'].'/plugin.class.php');
    ...
    function print_filters() {
        ...
        if (!empty($filters)) {
            ...
            if ($filterform->is_cancelled()) {
                redirect("$CFG->wwwroot/report/configreports/viewreport.php?id=".$this->config->id);
                die;
            }
            $filterform->display();
        }    
    ...
    function print_export_options($return = false) {
        ...
        if (!empty($export)) {
            ...
            $output .= get_string('downloadreport','report_configreports').': ';        
            foreach ($export as $e)
                if ($e) {
                    $output .= '<a href="'.$wwwpath.'&amp;download=1&amp;format='.$e.'"><img src="'.$CFG->wwwroot.'/report/configreports/export/'.$e.'/pix.gif" alt="'.$e.'">&nbsp;'.(strtoupper($e)).'</a>&nbsp;';
                }
            $output .= '</div>';
        }
    ...
    function evaluate_conditions($data,$logic) {
        ...
        require_once($CFG->dirroot.'/report/configreports/reports/evalwise.class.php');
    ...
    function get_graphs($finalreport) {
        ...
        if (!empty($graphs)) {
            $series = array();
            foreach ($graphs as $g) {
                require_once($CFG->dirroot.'/report/configreports/components/plot/'.$g['pluginname'].'/plugin.class.php');
    ...
    function get_calcs($finaltable, $tablehead) {
        ...
        if (!empty($calcs)) {
            ...
            foreach ($calcs as $calc) {
                require_once($CFG->dirroot.'/report/configreports/components/calcs/'.$calc['pluginname'].'/plugin.class.php');
    ...
    function elements_by_conditions($conditions) {
        ...
        foreach ($conditions['elements'] as $c) {
            require_once($CFG->dirroot.'/report/configreports/components/conditions/'.$c['pluginname'].'/plugin.class.php');
    ...
    function create_report() {
        ...
        if (!empty($ordering)) {
            foreach ($ordering as $o) {
                require_once($CFG->dirroot.'/report/configreports/components/ordering/'.$o['pluginname'].'/plugin.class.php');
        ...
        if ($rows) {
            foreach ($rows as $r) {
                ...
                foreach ($columns as $c) {
                    require_once($CFG->dirroot.'/report/configreports/components/columns/'.$c['pluginname'].'/plugin.class.php');
                    $classname = 'plugin_'.$c['pluginname'];
    ...
    function print_template($config) {
        ...
        echo $OUTPUT->pix_icon('print', get_string('printreport', 'report_configreports'), 'report_configreports');
        echo "&nbsp;<a href=\"javascript: printDiv('printablediv')\">".get_string('printreport','report_configreports')."</a>";    
        echo "</div>\n";
    }
    ...
    function print_report_page() {
        ...
        if ($this->finalreport->table && !empty($this->finalreport->table->data[0])) {    
            ...
            if (!empty($this->finalreport->calcs->data[0])) {
                echo '<br /><br /><br /><div class="centerpara"><b>'.get_string("columncalculations","report_configreports").'</b></div><br />';
            ...
            echo '<div class="centerpara">'.get_string('norecordsfound','report_configreports').'</div>';
        }
