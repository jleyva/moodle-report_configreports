 ...
 */
    ...
    $PAGE->set_url('/report/configreports/managereport.php', array('courseid'=>$course->id));
    ...
    if ($data = $mform->get_data()) {
        if ($xml = $mform->get_file_content('userfile')) {
            ...
            if (isset($data['report']['@']['version'])) {
                ...
                if (!$DB->insert_record('report_configreports',$newreport))
                    print_error('errorimporting');
                header("Location: $CFG->wwwroot/report/configreports/managereport.php?courseid={$course->id}");
                die;
            }
        }
    }    
    ...
    $title = get_string('reports','report_configreports');
    $navlinks = array();
    ...
    if ($reports) {
        $table = new stdclass;
        $table->head = array(get_string('name'),get_string('course'),get_string('type','report_configreports'),get_string('username'),get_string('edit'),get_string('download','report_configreports'));
        ...
        $strexport = get_string('exportreport','report_configreports');
        ...
        foreach ($reports as $r) {
            ...
            if (!empty($export)) {        
                foreach ($export as $e)
                    if ($e) {
                        $download .= '<a href="viewreport.php?id='.$r->id.'&amp;download=1&amp;format='.$e.'"><img src="'.$CFG->wwwroot.'/report/configreports/export/'.$e.'/pix.gif" alt="'.$e.'">&nbsp;'.(strtoupper($e)).'</a>&nbsp;&nbsp;';
                    }        
            }
    
            $table->data[] = array('<a href="viewreport.php?id='.$r->id.'">'.$r->name.'</a>',$coursename,get_string('report_'.$r->type,'report_configreports'), $owner, $editcell, $download);
        }
        ...
        echo $OUTPUT->heading(get_string('noreportsavailable','report_configreports'));
    }
    
    echo $OUTPUT->heading('<a href="'.$CFG->wwwroot.'/report/configreports/editreport.php?courseid='.$course->id.'">'.(get_string('addreport','report_configreports')).'</a>');
